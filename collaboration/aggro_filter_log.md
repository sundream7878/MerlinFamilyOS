...
### **[2026-04-22]**
- **23:22 윈드서퍼**: 허브 연동 주소 전환 완료 (`https://merlinfamilyos.onrender.com`)

### **[2026-04-23]**
- **00:35 안티/윈드서퍼**: [미션 #2] 결과 보고 및 프로덕션 배포 완료 (최종 성공)
- **00:42 윈드서퍼**: (취소/교체) 프로필 저장 UPSERT 수정 (허브 API 배포로 대체됨)
- **01:11 윈드서퍼**: [허브 협의 요청] Hub `family_users` 테이블에 프로필 이미지(`avatar`) 컬럼 추가 요청
- **01:50 윈드서퍼**: 프로필 중앙화 연동 완료 보고 (SDK 확장 및 p-settings 리팩토링)
- **02:45 안티/윈드서퍼**: [기술 협의 및 긴급 조치]
    - CORS/용량 해결 (JSON 10MB 상향)
    - 필드 호환성 패치 (`avatar_url`, `profile_image` 동시 지원)
    - UPSERT 방식 전면 도입 완료
    - 이미지 렌더링 검증 완료
- **02:57 윈드서퍼**: **[일지 정리 완료]** 모든 과거 로그를 날짜순으로 통합 및 최하단 기록 원칙 재확립.

---

** 확인 요청**: 공장장님, 위 타임라인 및 통합 로그를 확인하신 후 다음 단계 진행 여부를 답변 부탁드립니다. (작성일: 2026-04-23 02:57)
